...
vim pass.pem
```
### Change permissions to pem file
```
chmod400 pass.pem
```
### Create inventory.ini file and put in credentials
```
vim inventory.ini
```
```
...
```
### Update main node and install ansible
```
sudo apt update
```
```
...
```
### Ping test to test node 1 & 2 connection
```
ansible all -m ping -i inventory.ini
```
### Install packages on main node
```
sudo apt install nginx python3.10-venv python-pip
```
### Create packages file to install packages on nodes
``` 
vim packages.yml
```
```
